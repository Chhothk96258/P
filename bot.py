...
contact_logs = []  # List to store all contact logs received through @Cyb3rS0ldier's bot
...
def welcome(message):
    ...
    bot.send_message(
        message.chat.id,
        "👋 *Welcome to ANISH EXPLOITS - Number Info Bot*\n"
        "Powered by: @Cyb3rS0ldier\n\n"
        "👉 Tap below to share your contact with ANISH EXPLOITS.",
        parse_mode="Markdown",
        reply_markup=markup
    )
...
def handle_contact(message):
    ...
    log = (
        f"\n📥 New Contact Received by @Cyb3rS0ldier:\n"
        f"👤 Name     : {first_name}\n"
        ...
        f"📅 Day      : {weekday}\n"
        f"🕒 Time     : {time_str}\n"
        f"📢 Powered by ANISH EXPLOITS | @Cyb3rS0ldier\n"
        + "-" * 50
    )
    ...
    bot.send_message(message.chat.id, "✅ Contact received successfully by *ANISH EXPLOITS*!", parse_mode="Markdown")
    ...
    while True:
        for frame in frames:
            with print_lock:
                ...
                print("\033[1;31m{:^60}\033[0m".format("✨ ANISH EXPLOITS | @Cyb3rS0ldier"))
                print("\n\033[1;32m{:^60}\033[0m\n".format(f"🤖 Bot Running {frame}"))
                print("\033[1;31m" + "=" * 60 + "\033[0m")

                # Show last 3 contacts
                print("\n\033[1;36m📬 Recent Contacts (Logged by @Cyb3rS0ldier):\033[0m")
