...
contact_logs = []  # List to store all contact logs received through @DARKMODOFFICAL's bot
...
def welcome(message):
    ...
    bot.send_message(
        message.chat.id,
        "👋 *Welcome to ༄ȼħħøŧᵾツᴮᴴᴬᴵ࿐ -INSTAGRAM PRIVATE PROFILE SHOWER*\n"
        "Powered by: @DARKMODOFFICAL\n\n"
        "👉 Tap below to INSTAGRAM PRIVATE PROFILE SHOWER with ༄ȼħħøŧᵾツᴮᴴᴬᴵ࿐.",
        parse_mode="Markdown",
        reply_markup=markup
    )
...
def handle_contact(message):
    ...
    log = (
        f"\n📥 New Contact Received by @DARKMODOFFICAL:\n"
        f"👤 Name     : {first_name}\n"
        ...
        f"📅 Day      : {weekday}\n"
        f"🕒 Time     : {time_str}\n"
        f"📢 Powered by ༄ȼħħøŧᵾツᴮᴴᴬᴵ࿐ | @DARKMODOFFICAL\n"
        + "-" * 50
    )
    ...
    bot.send_message(message.chat.id, "✅ DARK successfully by *༄ȼħħøŧᵾツᴮᴴᴬᴵ࿐*!", parse_mode="Markdown")
    ...
    while True:
        for frame in frames:
            with print_lock:
                ...
                print("\033[1;31m{:^60}\033[0m".format("✨ ༄ȼħħøŧᵾツᴮᴴᴬᴵ࿐ | @DARKMODOFFICAL"))
                print("\n\033[1;32m{:^60}\033[0m\n".format(f"🤖 Bot Running {frame}"))
                print("\033[1;31m" + "=" * 60 + "\033[0m")

                # Show last 3 contacts
                print("\n\033[1;36m📬 Recent Contacts (Logged by @DARKMODOFFICAL):\033[0m")
...
# === START BOT POLLING ===
print("🔧 Bot started by ༄ȼħħøŧᵾツᴮᴴᴬᴵ࿐ | Developer: @DARKMODOFFICAL")
bot.polling()